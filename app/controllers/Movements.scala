...
class Movements @Inject() (
  ...
  val collection = db[JSONCollection]("movements")
  ...
  def index = Action.async { implicit request =>
    ...
    val found = collection.find(query).cursor[Movement]
    // build (asynchronously) a list containing all the articles
    found.collect[List]().map { movements =>
      Ok(views.html.movements(movements, activeSort))
    }.recover {
      case e =>
        e.printStackTrace()
  ...
  def showCreationForm = Action { request =>
    ...
    Ok(views.html.editMovement(None, Movement.form, None))
  }
  
   def create = Action.async { implicit request =>
    implicit val messages = messagesApi.preferred(request)

    Movement.form.bindFromRequest.fold(
      errors => Future.successful(
        Ok(views.html.editMovement(None, errors, None))),

      // if no error, then insert the article into the 'articles' collection
      movement => collection.insert(movement.copy(
        id = movement.id.orElse(Some(UUID.randomUUID().toString)),
        dateMovement = Some(new DateTime()))
      ).map(_ => Redirect(routes.Movements.index))
    )
  }
/*
  def showEditForm(id: String) = Action.async { request =>
    // get the documents having this id (there will be 0 or 1 result)
    val futureMovement = collection.find(Json.obj("_id" -> id)).one[Movement]
